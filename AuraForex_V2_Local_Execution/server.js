...
server.listen(PORT, () => {
  ...
  prisma.$connect()
    .then(() => {
      ...
      // ── INICIAR SIMULADOR AUTOMÁTICO DE TRADES PAMM (90 segundos) ──
      console.log("[DIAGNOSTIC] 💳 Iniciando Simulador Automático de Trades PAMM (Taxas de Gás)...");
      setInterval(async () => {
        try {
          // 1. Procurar todas as contas PAMM ativas
          const pammAccounts = await prisma.pammAccount.findMany({
            where: { isActive: true }
          });

          for (const account of pammAccounts) {
            try {
              // 2. Verificar se o usuário tem saldo de gás (walletBalance > 0)
              const user = await prisma.user.findUnique({
              ...
              if (!user || user.walletBalance <= 0) {
                console.log(`[PAMM-SIMULATOR] Cópia suspensa para User ${account.userId} devido a saldo de gás zerado ($${user?.walletBalance ?? 0}).`);
                continue;
              }

              // 3. Simular fechamento de ordem
              ...
              let pnlAmount = 0;
              if (isWin) {
                // Vitória: ganho entre $15.00 e $45.00
                pnlAmount = parseFloat((15 + Math.random() * 30).toFixed(2));
              } else {
                // Derrota: perda entre $5.00 e $18.00
                pnlAmount = -parseFloat((5 + Math.random() * 13).toFixed(2));
              }

              const newBalance = Math.max(10.0, account.balance + pnlAmount);
              const newEquity = newBalance; // Manter equity sincronizada

              const updatedProfit = isWin ? account.totalProfit + pnlAmount : account.totalProfit;
              const updatedLoss = !isWin ? account.totalLoss + Math.abs(pnlAmount) : account.totalLoss;

              // 4. Se for vitória, calcula a dedução da taxa de performance
              let feeDeducted = 0;
              let newWalletBalance = user.walletBalance;

              if (isWin) {
                const feePct = user.settings?.pammPerformanceFeePct ?? 30.0;
                feeDeducted = parseFloat((pnlAmount * (feePct / 100.0)).toFixed(2));
                newWalletBalance = Math.max(0.0, user.walletBalance - feeDeducted);

                // Registrar WalletTransaction no formato estrito: (data) (valor) (taxa de serviço)
                const d = new Date();
                const dateStr = `${String(d.getDate()).padStart(2, '0')}/${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
                const txDescription = `(${dateStr}) ($${feeDeducted.toFixed(2)}) (Taxa de Serviço PAMM - Lucro ${randomPair})`;
                ...
                console.log(`[PAMM-SIMULATOR] Gás deduzido para User ${user.id}: -$${feeDeducted} (${feePct}%) da transação +$${pnlAmount} em ${randomPair}`);
              }

              // 5. Atualizar conta PAMM no banco
              ...
              console.log(`[PAMM-SIMULATOR] User ${user.id} PAMM atualizado: P&L: $${pnlAmount >= 0 ? '+' : ''}${pnlAmount} em ${randomPair}. Novo Saldo PAMM: $${newBalance.toFixed(2)}. Gás Restante: $${newWalletBalance.toFixed(2)}`);

            } catch (innerErr) {
              console.error(`[PAMM-SIMULATOR-INNER-ERROR] Conta de User ${account.userId}:`, innerErr.message);
            }
          }
        } catch (err) {
          console.error("[PAMM-SIMULATOR-ERROR]:", err.message);
        }
      }, 90000);
    })
