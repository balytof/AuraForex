...
const targetRegex = /tr\.innerHTML = \`[\s\S]*?<td>\$\{statusBadge\}<\/td>\s*\`;/m;
const newStr = `                
...
if (targetRegex.test(txt)) {
  txt = txt.replace(targetRegex, newStr);
  console.log("Admin table logic replaced successfully.");
} else {
  console.log("Admin table target NOT found");
}

const funcRegex = /    loadStats\(\);\s*loadSystemSettings\(\);/m;
const funcNew = `    async function removePammAccount(userId) {
...
if (funcRegex.test(txt)) {
  txt = txt.replace(funcRegex, funcNew);
  console.log("Admin func replaced successfully.");
} else {
  console.log("Admin func target NOT found");
}
