...
run();
`;
fs.writeFileSync('./test_with_env3.js', scriptContent);

conn.on('ready', () => {
    conn.sftp((err, sftp) => {
        if (err) throw err;
        sftp.fastPut('./test_with_env3.js', '/root/AuraForex/AuraForex_V2_Local_Execution/test_with_env3.js', (err) => {
            if (err) throw err;
            conn.exec('cd /root/AuraForex/AuraForex_V2_Local_Execution && node test_with_env3.js', (err, stream) => {
                if (err) throw err;
                let output = '';
