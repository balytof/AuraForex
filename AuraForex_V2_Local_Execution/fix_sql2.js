...
conn.on('ready', () => {
    ...
    SET "totalGasPaid" = 10 
    WHERE "userId" IN (SELECT id FROM "User" WHERE email='balytof@gmail.com');
    ...
    const cmd = `psql postgresql://aura_admin:%40Infomoi2023@localhost:5432/auraforex -c "${sql.replace(/\n/g, ' ')}"`;
