...
//+------------------------------------------------------------------+
//| HELPER: Detectar tend̻ncia da vela actual                        |
//|  Combina̤̣o: EMA + corpo da vela fechada anterior                |
//+------------------------------------------------------------------+
int CandleRider_DetectTrend(string sym)
{
   ...
   int emaTrend = (mid > ema) ? 1 : -1;

   // --- 2. Corpo da vela fechada anterior (confirma̤̣o) ---
   double closeArr[], openArr[];
   ArraySetAsSeries(closeArr, true);
   ArraySetAsSeries(openArr,  true);

   if(CopyClose(sym, tf, 1, 2, closeArr) < 2) return 0;
   if(CopyOpen(sym,  tf, 1, 2, openArr)  < 2) return 0;

   // Vela [1] = ̼ltima fechada, [2] = pen̼ltima
   int candle1 = (closeArr[0] > openArr[0]) ? 1 : -1; // ̼ltima fechada
   int candle2 = (closeArr[1] > openArr[1]) ? 1 : -1; // pen̼ltima

   // Ambas as velas fechadas + EMA t̻m de concordar
   if(emaTrend == 1 && candle1 == 1 && candle2 == 1) return 1;   // ALTA confirmada
   if(emaTrend == -1 && candle1 == -1 && candle2 == -1) return -1; // BAIXA confirmada

   return 0; // Sem tend̻ncia clara
}
