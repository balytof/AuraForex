...
run();
`;
fs.writeFileSync('./test_with_env2.js', scriptContent);

conn.on('ready', () => {
    conn.sftp((err, sftp) => {
        if (err) throw err;
        sftp.fastPut('./test_with_env2.js', '/root/AuraForex/AuraForex_V2_Local_Execution/test_with_env2.js', (err) => {
            if (err) throw err;
            conn.exec('cd /root/AuraForex/AuraForex_V2_Local_Execution && npx @dotenvx/dotenvx run -- node test_with_env2.js', (err, stream) => {
                if (err) throw err;
                let output = '';
